...
function getInput(controls) {
    ...
    controls.section('Search Queries', (section) => {
        ...
        section
            .textarea('bulk_queries', {
                isRequired: false,
                label: 'Bulk Edit (one query per line)',
                placeholder: 'Hotel di Padang\nRestaurant di Padang',
                isShown: (data) => {
                    return data.use_bulk_edit === true;
                },
                isDisabled: (data) => {
                    // Disable if location is provided
                    return data.location && data.location.trim() !== '';
            ...
            .text('location', {
                isRequired: (data) => {
                    ...
                    return (!data.queries || data.queries.length === 0 || data.queries[0] === '') && 
                           (!data.use_bulk_edit || !data.bulk_queries || data.bulk_queries.trim() === '');
                },
                label: 'Location Name',
                placeholder: 'Padang',
