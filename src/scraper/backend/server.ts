...
try {
    ...
    const csvFilePath = path.join(process.cwd(), 'inputs', 'googlemaps_category.csv');
    const csvData = fs.readFileSync(csvFilePath, 'utf-8');
    googleMapsCategories = csvData.split(/\r?\n/).map(category => category.trim()).filter(category => category);
} catch (error) {
...
function splitTaskByQuery(data: any): any[] {
    ...
    if (data.use_categories && data.category_location && googleMapsCategories.length > 0) {
        ...
        const bulkQueriesList = bulkQueriesText.split(/r?n/).map((q: string) => q.trim()).filter((q: string) => q);
        
        allQueries = [...manualQueries, ...bulkQueriesList];
    }
    
    // Remove duplicates and empty queries
    const uniqueQueries = Array.from(new Set(allQueries.filter(q => q && q.trim() !== '')));
    ...
    if (uniqueQueries.length === 0) {
        ...
        if (data.use_categories && (!data.category_location || data.category_location.trim() === '')) {
...
Server.configure({
  ...
  rightHeader: {
    text: 'Ogi',
    link: 'https://github.com/baobei23', // Update with your repo
  },
});
