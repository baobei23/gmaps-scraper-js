...
function createAsyncTask(data: any) {
  return fetch(
    { route: 'createAsyncTask', message: 'Starting Task', silentOnError: true },
    data,
  );
}
...
function isAnyTaskUpdated(
  pending_task_ids: any[],
  progress_task_ids: any[],
  all_tasks: any[],
) {
  return fetch(
    { route: 'isAnyTaskUpdated', silent: true },
    {
      pending_task_ids,
      progress_task_ids,
      all_tasks,
...
function isTaskUpdated(taskId: number, lastUpdated: string, status: string) {
  return fetch(
    { route: 'isTaskUpdated', silent: true },
    {
      task_id: taskId,
...
function abortTask(task_id: number, page: number) {
  return fetch(
    { route: 'patchTask', message: 'Aborting...' },
    { page },
    {
      action: 'abort',
      task_ids: [task_id],
...
function bulkAbortTask(task_ids: number[], page: number) {
  return fetch(
    { route: 'patchTask', message: 'Aborting...' },
    { page },
    {
...
function deleteTask(task_id: number, page: number) {
  return fetch(
    { route: 'patchTask', message: 'Deleting...' },
    { page },
    {
      action: 'delete',
      task_ids: [task_id],
...
function bulkDeleteTask(task_ids: number[], page: number) {
  return fetch(
    { route: 'patchTask', message: 'Deleting...' },
    { page },
    {
...
function downloadTaskResults(taskId: number, data: any = {}) {
  return fetch(
    { route: 'downloadTaskResults', message: 'Downloading...' },
    taskId,
    data,
  );
}
...
function getUiTaskResults(
  taskId: number,
  data: any = {},
  force_apply_first_view = false,
) {
  const query_params = force_apply_first_view
    ? { force_apply_first_view: `${force_apply_first_view}` }
    : {};
  const newLocal = fetch(
    { route: 'getUiTaskResults', silent: true },
    taskId,
    query_params,
    data,
  );
